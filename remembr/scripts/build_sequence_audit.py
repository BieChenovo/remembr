...
renderRetrievals=q=>{{const enriched=(q.retrieval_trace||[]).map(call=>{{const control=[call.controller_turn_id!==undefined?`Turn ${{call.controller_turn_id}}`:null,call.tool_batch_id?`Batch ${{call.tool_batch_id}} (size=${{call.tool_batch_size}})`:null,call.retrieval_kind||null,call.duplicate_blocked?'DUPLICATE BLOCKED':null].filter(Boolean).join(' · ');return {{...call,query:call.raw_query??call.query,retrieval_method:[call.retrieval_method,control].filter(Boolean).join(' · '),score_name:call.duplicate_blocked?'not executed':call.score_name,returned_context:call.blocked_reason||call.returned_context}}}});return legacyRenderRetrievals({{...q,retrieval_trace:enriched}})}};
